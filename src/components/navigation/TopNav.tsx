...
export const TopNav: React.FC<TopNavProps> = ({ currentView, onSelectView, overdueCount = 0 }) => {
  ...
  const btnStyle = (active: boolean) =>
    `px-3 py-1.5 rounded-control text-xs font-bold transition-colors flex items-center gap-1.5 ${
      active ? 'bg-cobalt text-white shadow-sm' : 'bg-wash text-ink hover:bg-line'
    }`;
    ...
    <nav aria-label="Main Navigation" className="hidden md:flex items-center gap-1.5 border-b border-line pb-2 flex-wrap">
      ...
      <div className="inline-flex items-center p-0.5 rounded-control bg-wash border border-line">
        <button
          ...
          className={`px-2.5 py-1.5 rounded-control text-xs font-bold transition-colors flex items-center gap-1.5 ${currentView === 'pipeline' ? 'bg-cobalt text-white shadow-sm' : 'text-sub hover:text-ink'}`}
        >
        ...
        <button
          ...
          className={`px-2.5 py-1.5 rounded-control text-xs font-bold transition-colors flex items-center gap-1.5 ${currentView === 'board' ? 'bg-cobalt text-white shadow-sm' : 'text-sub hover:text-ink'}`}
        >
      ...
      <button type="button" onClick={() => onSelectView('follow_ups')} className={btnStyle(currentView === 'follow_ups')}>
        ...
        {overdueCount > 0 && (
          <span className="px-1.5 py-0.2 rounded-full bg-overdue text-white text-[10px] font-bold">{overdueCount}</span>
        )}
      </button>
      ...
      <div ref={dropdownRef} className="relative group">
        ...
        <div
          className={`absolute left-0 top-full mt-1.5 w-44 bg-card border border-line rounded-card shadow-lg p-1 z-30 transition-all ${
            isOpen ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-95 pointer-events-none md:group-hover:opacity-100 md:group-hover:scale-100 md:group-hover:pointer-events-auto'
          }`}
        >
          {DEAL_DESKS.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-control text-xs font-semibold transition-colors ${currentView === id ? 'bg-cobalt text-white' : 'text-sub hover:text-ink hover:bg-wash'}`}
            >
